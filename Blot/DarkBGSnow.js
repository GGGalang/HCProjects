...
function drawBG() {
  //draws the black background, basically a snake
  ...
  const bgs = bg.lines();
  drawLines(bgs, { width: 4, stroke: "black" })
  ...
  const cgs = cg.lines();
  drawLines(cgs, { width: 4, stroke: "white" })
  ...
  drawLines(clouds, { width: 20, stroke: "white" })
}
drawBG();
...
for (let x = 0; x < 30; x++) {
  ...
  for (let i = 0; i < armNums; i++) {
    ...
    bt.merge(shaft)
    bt.simplify(shaft)
    drawLines(shaft, { width: 0.1, stroke: "white" })
  }
}